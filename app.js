...
//  API HELPER
// =====================================================
// Warera uses tRPC — procedure names are camelCase like "user.getLite"
// Input is wrapped as { json: input } per tRPC batch spec
async function trpc(procedure, input = {}) {
  const params = encodeURIComponent(JSON.stringify({ 0: { json: input } }));
  const url = `${BASE}/${procedure}?batch=1&input=${params}`;
  ...
  // tRPC response: data[0].result.data.json  OR  data[0].result.data
...
async function loadUserInfo() {
  ...
  if (!user) {
    // Correct procedure: user.getLite, input: { userId: "123" }
    user = await trpc('user.getLite', { userId: cfg.userId });
    setCache(cKey, user);
  }
  renderUserInfo(user);
}
...
async function loadUserCompanies() {
  ...
  if (!companies) {
    try {
      // correct: company.getCompanies with userId filter
...
async function loadUserTxns() {
  ...
  if (!txns) {
    ...
    // correct procedure: transaction.getPaginated
    const raw = await trpcPaginated('transaction.getPaginated', { userId: cfg.userId, limit: 50 }, 10);
    txns = raw.filter(t => {
...
async function loadMuTxns() {
  ...
  if (!txns) {
    const weekAgo = Date.now() - WEEK_MS;
    const raw = await trpcPaginated('transaction.getPaginated', { muId: cfg.muId, limit: 50 }, 10);
    txns = raw.filter(t => {
      const ts = t.createdAt || t.timestamp || t.date;
      return !ts || new Date(ts).getTime() > weekAgo;
    });
...
async function loadMuMembers() {
  ...
  if (!members) {
    // Get member IDs from MU, then fetch user details
    const mu = await trpc('mu.getById', { muId: cfg.muId });
    const memberIds = mu?.memberIds || mu?.members?.map(m => m.userId || m.id) || [];
    if (!memberIds.length) { members = []; }
    else {
      const chunks = chunkArray(memberIds, 50);
      members = [];
      for (const chunk of chunks) {
        try {
          // correct: user.getMany with userIds array
          const result = await trpc('user.getMany', { userIds: chunk });
          members = members.concat(Array.isArray(result) ? result : []);
        } catch {
          for (const uid of chunk) {
            try { members.push(await trpc('user.getLite', { userId: uid })); } catch {}
          }
        }
      }
    }
