...
async function trpcPaginated(procedure, input = {}, pages = 5) {
  ...
  for (let i = 0; i < pages; i++) {
    ...
    if (!page) break;
    const batch = page.items || page.data || [];
    items = items.concat(batch);
    if (!page.nextCursor && !page.next_cursor) break;
...
async function loadUserCompanies() {
  ...
  if (!companies) {
    try {
      companies = await trpcPaginated('company.getCompanies', { userId: cfg.userId }, 3);
...
async function loadUserTxns() {
  ...
  if (!txns) {
    ...
    const raw = await trpcPaginated('transaction.getTransactions', { userId: cfg.userId, limit: 50 }, 10);
    txns = raw.filter(t => {
...
async function loadMuInfo() {
  ...
  if (!mu) {
    mu = await trpc('mu.getMuById', { muId: cfg.muId });
    setCache(cKey, mu);
  }
  renderMuInfo(mu);
}
...
async function loadMuTxns() {
  ...
  if (!txns) {
    const weekAgo = Date.now() - WEEK_MS;
    const raw = await trpcPaginated('transaction.getTransactions', { muId: cfg.muId, limit: 50 }, 10);
    txns = raw.filter(t => {
      const ts = t.createdAt || t.timestamp || t.date;
      return !ts || new Date(ts).getTime() > weekAgo;
    });
...
async function loadMuMembers() {
  ...
  if (!members) {
    ...
    const mu = await trpc('mu.getMuById', { muId: cfg.muId });
    ...
    if (!memberIds.length) { members = []; }
      ...
      for (const chunk of chunks) {
        try {
          // Use paginated user endpoint filtered by MU
          const page = await trpc('user.getManyUsers', { userIds: chunk });
          members = members.concat(page || []);
        } catch {
          // Fallback: fetch one at a time (slower)
          for (const uid of chunk) {
            try { members.push(await trpc('user.getUserLite', { userId: uid })); } catch {}
          }
        }
      }
    }
